...
xml_path = 'jlr_prismatic.xml'    #xml file (assumes this is in the same folder as this file)
...
initial_joint_val = np.zeros(5)

for i in range(5):
    data.qpos[i] = initial_joint_val[i]

data.qpos[1] =np.pi/6.0 
data.qpos[2] =-0.2
...
filepath1 = os.path.join(dirname + "/" + "xyz_file_pris.csv")
filepath2 = os.path.join(dirname + "/" + "jang_file_pris.csv")
filepath3 = os.path.join(dirname + "/" + "delq_file_pris.csv")
filepath4 = os.path.join(dirname + "/" + "err_file_pris.csv")
filepath5 = os.path.join(dirname + "/" + "jvel_file_pris.csv")
...
jang_file.write("Timestamp, j1, j2, l3, j4, j5\n")
delq_file.write("Timestamp, jdel_1, del_j2, del_l3, del_j4, del_j5\n")
err_file.write("Timestamp, delX, delY, delZ\n")
jvel_file.write("Timestamp, jvel_1, jvel_2, lvel_3, jvel_4, jvel_5\n")
...
while True:
    ...
    while not glfw.window_should_close(window):
        ...
        pos_ee = data.site_xpos[0]
        j = np.zeros((3,5))
        mj.mj_jac(model, data, j, None, pos_ee, 5)
        ...
        for k in range(5):
            if (k == 2):
                while(abs(dq[k][0] >= 0.009)):
                    dq[k][0] /= 10
            else:
                while(abs(dq[k][0]) >= 0.0085):
                    dq[k][0] /= 10
        
        del_q = f'{round(t0,2)},{dq[0][0]},{dq[1][0]},{dq[2][0]},{dq[3][0]},{dq[4][0]}\n'

        q = f'{round(t0,2)},{data.qpos[0]},{data.qpos[1]},{data.qpos[2]},{data.qpos[3]},{data.qpos[4]}\n'
        
        xyz = f'{round(t0,2)},{pos_ee[0]},{pos_ee[1]},{pos_ee[2]}\n'
        
        err = f'{round(t0,2)},{goal[0]-pos_ee[0]},{goal[1]-pos_ee[1]},{goal[2]-pos_ee[2]}\n'
        
        jvel = f'{round(t0,2)},{dq[0][0]/dt},{dq[1][0]/dt},{dq[2][0]/dt},{dq[3][0]/dt},{dq[4][0]/dt}\n'
        ...
        glfw.poll_events()
    if (flag == True):
        time.sleep(10)   # model will be displayed till 5 seconds after goal is reached
